...
if (count($parts) === 3 && $parts[2] === 'series') {
  $stmt = pacs_db()->prepare('SELECT * FROM pacs_series WHERE study_uid = ? ORDER BY series_number ASC, id ASC');
...
if (count($parts) === 5 && $parts[2] === 'series' && $parts[4] === 'instances') {
  ...
  $stmt = pacs_db()->prepare('SELECT * FROM pacs_instances WHERE study_uid = ? AND series_uid = ? ORDER BY instance_number ASC, id ASC');
