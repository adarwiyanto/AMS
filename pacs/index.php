...
$stats = [
  'patients' => (int)db()->query('SELECT COUNT(*) FROM pacs_patients')->fetchColumn(),
  'studies' => (int)db()->query('SELECT COUNT(*) FROM pacs_studies')->fetchColumn(),
  'series' => (int)db()->query('SELECT COUNT(*) FROM pacs_series')->fetchColumn(),
  'instances' => (int)db()->query('SELECT COUNT(*) FROM pacs_instances')->fetchColumn(),
];
...
<div class="card">
  ...
  <div class="muted">Modul RIS-PACS Lite (DICOMWeb QIDO-RS + WADO-URI).</div>
...
<div class="card">
  ...
  <a class="btn secondary" href="<?= e(url('/ohif/')) ?>" target="_blank" rel="noopener">Viewer OHIF</a>
</div>
