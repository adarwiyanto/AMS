...
function pacs_store_one_file(string $tmpPath, string $originalName, array $metaMap, int $saved, int $skipped, array &$errors): array {
  ...
  $dup = pacs_db()->prepare('SELECT id FROM pacs_instances WHERE sop_uid = ? LIMIT 1');
  $dup->execute([$sopUid]);
  if ($dup->fetch()) {
  ...
  pacs_db()->prepare('INSERT INTO pacs_patients (patient_id, patient_name, birth_date, sex, created_at) VALUES (?,?,?,?,?) ON DUPLICATE KEY UPDATE patient_name=VALUES(patient_name)')
    ->execute([$patientId, $patientName, '', '', now_dt()]);

  pacs_db()->prepare('INSERT INTO pacs_studies (study_uid, patient_id, study_date, accession, study_desc, modalities, num_series, num_instances, created_at) VALUES (?,?,?,?,?,?,?,?,?)
  ...
  pacs_db()->prepare('INSERT INTO pacs_series (series_uid, study_uid, modality, series_number, series_desc, body_part, num_instances) VALUES (?,?,?,?,?,?,?)
    ON DUPLICATE KEY UPDATE modality=VALUES(modality), num_instances=num_instances+1')
    ->execute([$seriesUid, $studyUid, $modality, 0, '', '', 1]);

  pacs_db()->prepare('UPDATE pacs_studies SET num_series = (SELECT COUNT(*) FROM pacs_series WHERE study_uid = ?), num_instances = (SELECT COUNT(*) FROM pacs_instances WHERE study_uid = ?) + 1 WHERE study_uid = ?')
    ->execute([$studyUid, $studyUid, $studyUid]);

  pacs_db()->prepare('INSERT INTO pacs_instances (sop_uid, series_uid, study_uid, patient_id, instance_number, image_rows, image_cols, frames) VALUES (?,?,?,?,?,?,?,?)')
    ->execute([$sopUid, $seriesUid, $studyUid, $patientId, 0, 0, 0, 1]);

  pacs_db()->prepare('INSERT INTO pacs_files (sop_uid, rel_path, file_size, sha256, created_at) VALUES (?,?,?,?,?)')
